...
static uint32_t DEVICE_HEADSET_TX            = 3; //headset_mono_tx
static uint32_t DEVICE_SPEAKER_TX            = 4;//speaker_mono_tx
static uint32_t DEVICE_HEADSET_RX            = 5; //headset_stereo_rx
static uint32_t DEVICE_TTY_HEADSET_MONO_TX   = 6; //tty_headset_mono_tx
static uint32_t DEVICE_TTY_HEADSET_MONO_RX   = 7; //tty_headset_mono_rx
static uint32_t DEVICE_SPEAKER_HEADSET_RX    = 8; //headset_stereo_speaker_stereo_rx

static uint32_t DEVICE_HS_STEREO_LB             	 = 9;
static uint32_t DEVICE_SPEAKER_STEREO_LB             = 10;
static uint32_t DEVICE_LINEIN_PRI_TX             	 = 11;
static uint32_t DEVICE_FTM_HEADSET_MONO_RX           = 12;
static uint32_t DEVICE_FTM_HEADSET_MONO_L_RX         = 13;
static uint32_t DEVICE_FTM_HEADSET_MONO_R_RX         = 14;
static uint32_t DEVICE_FTM_HEADSET_MONO_DIFF_RX      = 15;
static uint32_t DEVICE_FTM_SPKR_MONO_RX              = 16;
static uint32_t DEVICE_FTM_SPKR_L_RX             	 = 17;
static uint32_t DEVICE_FTM_SPKR_R_RX             	 = 18;
static uint32_t DEVICE_FTM_SPKR_MONO_DIFF_RX         = 19;
static uint32_t DEVICE_FTM_LINEIN_L_TX               = 20;
static uint32_t DEVICE_FTM_LINEIN_R_TX               = 21;
static uint32_t DEVICE_FTM_AUX_OUT_RX             	 = 22;
static uint32_t DEVICE_FTM_DMIC1_LEFT_TX             = 23;
static uint32_t DEVICE_FTM_DMIC1_RIGHT_TX            = 24;
static uint32_t DEVICE_FTM_DMIC1_L_AND_R_TX          = 25;
static uint32_t DEVICE_FTM_DMIC2_LEFT_TX          	 = 26;
static uint32_t DEVICE_FTM_DMIC2_RIGHT_TX            = 27;
static uint32_t DEVICE_FTM_DMIC2_L_AND_R_TX       	 = 28;
static uint32_t DEVICE_FTM_HANDSET_MIC1_AUX_IN    	 = 29;
static uint32_t DEVICE_FTM_HANDSET_MIC_ADIE_LP_TX 	 = 30;
static uint32_t DEVICE_FTM_HEADSET_MIC_ADIE_LP_TX 	 = 31;
static uint32_t DEVICE_FTM_HANDSET_ADIE_LP_RX     	 = 32;
static uint32_t DEVICE_FTM_HEADSET_L_ADIE_LP_RX   	 = 33;
static uint32_t DEVICE_FTM_HEADSET_R_ADIE_LP_RX   	 = 34;
static uint32_t DEVICE_FTM_SPK_L_ADIE_LP_RX       	 = 35;
static uint32_t DEVICE_FTM_SPK_R_ADIE_LP_RX       	 = 36;
static uint32_t DEVICE_FTM_SPK_ADIE_LP_RX         	 = 37;
static uint32_t DEVICE_HANDSET_MIC1_HANDSET_MIC2     = 38;
static uint32_t DEVICE_BT_SCO_RX         		     = 39; //bt_sco_rx
static uint32_t DEVICE_BT_SCO_TX        		     = 40; //bt_sco_tx
static uint32_t DEVICE_FMRADIO_STEREO_TX  		     = 41;
static uint32_t DEVICE_FMRADIO_STEREO_RX    		 = 42;
static uint32_t DEVICE_MI2S_SD0_RX           	  	 = 43;
static uint32_t DEVICE_MI2S_SD1_RX           	  	 = 44;
static uint32_t DEVICE_MI2S_SD2_RX           	  	 = 45;
static uint32_t DEVICE_UPLINK_RX             		 = 46;
static uint32_t DEVICE_HDMI_STERO_RX       		     = 47; //hdmi_stereo_rx
static uint32_t DEVICE_HDMI_PASS_THROUGH             = 48;

static uint32_t DEVICE_FMRADIO_HANDSET_RX    = 49; //fmradio_handset_rx
static uint32_t DEVICE_FMRADIO_HEADSET_RX    = 50; //fmradio_headset_rx
static uint32_t DEVICE_FMRADIO_SPEAKER_RX    = 51; //fmradio_speaker_rx
static uint32_t DEVICE_DUALMIC_HANDSET_TX    = 52; //handset_dual_mic_endfire_tx
static uint32_t DEVICE_DUALMIC_SPEAKER_TX    = 53; //speaker_dual_mic_endfire_tx
static uint32_t DEVICE_ANC_HEADSET_STEREO_RX = 54; //ANC RX
...
//ZTE
            else if(strcmp((char* )name[i], "hs_stereo_lb") == 0)
                index = DEVICE_HS_STEREO_LB;
            else if(strcmp((char* )name[i], "speaker_stereo_lb") == 0)
                index = DEVICE_SPEAKER_STEREO_LB;
            else if(strcmp((char* )name[i], "linein_pri_tx") == 0)
                index = DEVICE_LINEIN_PRI_TX;
            else if(strcmp((char* )name[i], "handset_mic1_handset_mic2") == 0)
                index = DEVICE_HANDSET_MIC1_HANDSET_MIC2;
            else if(strcmp((char* )name[i], "mi2s_sd0_rx") == 0)
                index = DEVICE_MI2S_SD0_RX;
            else if(strcmp((char* )name[i], "mi2s_sd1_rx") == 0)
                index = DEVICE_MI2S_SD1_RX;
            else if(strcmp((char* )name[i], "mi2s_sd2_rx") == 0)
                index = DEVICE_MI2S_SD2_RX;
            else if(strcmp((char* )name[i], "uplink_rx") == 0)
                index = DEVICE_UPLINK_RX;
            else if(strcmp((char* )name[i], "hdmi_pass_through") == 0)
                index = DEVICE_HDMI_PASS_THROUGH;
                                   ...
                                   int mode, bool mic_mute)
{
    ...
    if(device == SND_DEVICE_HANDSET) {
        ...
        ALOGV("In HANDSET(NO handset redirected to speaker)");
    }
        ...
        if (inputDevice != 0) {
            if (inputDevice & AudioSystem::DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
                ...
                if (outputDevices & AudioSystem::DEVICE_OUT_EARPIECE) {
                    ALOGI("Routing audio to Handset Redirected to speaker\n");
...
#endif
          if(outputDevices & AudioSystem::DEVICE_OUT_EARPIECE){
            ALOGI("Routing audio to HandsetEarpice redirected to Speaker\n");
            sndDevice = SND_DEVICE_SPEAKER;
            audProcess = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        }
    }
